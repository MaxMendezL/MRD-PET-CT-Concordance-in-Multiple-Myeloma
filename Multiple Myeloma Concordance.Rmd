---
title: "MRD/PET-CT Concordance — Analysis Report"
...
```{r flags-fh, echo=FALSE, message=FALSE, warning=FALSE}
fh_table_ok <- FALSE

# ST1 is only possible if we can find usable Study/a/b/c/d/n somewhere
src <- NULL
if (exists("tab2") && is.data.frame(tab2) && nrow(tab2) > 0) {
  src <- tab2
} else if (file.exists(file.path("tables","Table_2_Concordance_Augmented.csv"))) {
  tmp <- try(readr::read_csv(file.path("tables","Table_2_Concordance_Augmented.csv"), show_col_types = FALSE),
             silent = TRUE)
  if (!inherits(tmp, "try-error")) src <- tmp
...
if (is.data.frame(src) && nrow(src) > 0) {
  ...
  names(src) <- gsub("−","-", names(src), fixed = TRUE)
  ...
  if (!any(is.na(needed))) {
    ...
    fh_table_ok <- nrow(df2) > 0
  }
}

# FH figure is produced from ST1 (in-memory or CSV), so same flag is fine
fh_fig_ok <- fh_table_ok
...
This report reproduces the MRD/PET-CT concordance and prognostic analyses from paired study-level data in multiple myeloma. Where needed, reported percentages were converted back to cell counts and reconciled to the reported sample size. The main analysis includes paired MRD and PET-CT assessments at the same clinical landmark, with additional sensitivity analyses for stricter timing and reconstruction assumptions.All inputs are harmonised: a = MRD−/PET–CT+, b = MRD−/PET–CT−, c = MRD+/PET–CT−, d = MRD+/PET–CT+.

Analysis sets: (i) Primary (paired MRD and PET–CT assessments at the same clinical landmark); (ii) Strict pairing sensitivity set (Δ ≤ 30 days between MRD and PET–CT); and (iii) prespecified sensitivity analyses based on reconstruction assumptions and risk-of-bias exclusions.

Agreement metrics: Per study we report observed agreement (pooled concordant proportion), Cohen’s κ (with standard error and 95% confidence interval), McNemar’s test, and complementary prevalence-robust agreement measures (Gwet’s AC1 and prevalence-adjusted bias-adjusted κ [PABAK]). Study-level κ is pooled using random-effects meta-analysis (restricted maximum likelihood) with Knapp–Hartung adjustment. Directional discordance is defined as the log-odds of MRD−/PET–CT+ versus MRD+/PET–CT− discordant classifications, using a continuity correction where needed; 95% prediction intervals are reported when estimable.

Fréchet–Hoeffding κ bounds: Identification regions are derived for studies with incomplete 2×2 information and presented as supplementary graphical analyses to support cautious interpretation where κ is only weakly identified.

Survival (progression-free survival, PFS): The primary prognostic contrast is dual negativity (MRD−/PET–CT−) versus all other MRD/PET–CT categories. Hazard ratios (HRs) are pooled using random-effects meta-analysis (restricted maximum likelihood) with Knapp–Hartung adjustment. Small-study asymmetry assessments are interpreted descriptively given the limited number of cohorts.

Primary pooled 2×2 matrix (paired same-landmark set): MRD−/PET–CT+ 145 (12.7%), MRD−/PET–CT− 499 (43.8%), MRD+/PET–CT− 310 (27.2%), and MRD+/PET–CT+ 184 (16.2%); total paired n=1,138. Observed agreement = 60.0%, pooled κ = 0.14 (95% CI 0.03–0.25), and pooled directional discordance log-odds = −0.83 (95% CI −1.87 to 0.21). In five studies reporting PFS by joint MRD/PET–CT status, the pooled HR for dual-negative versus all other categories is 0.34 (95% CI 0.22–0.51).

Key outputs: Table 1 (study characteristics); Figure 1 (PRISMA flow diagram); Figure 2A (agreement meta-analysis, Cohen’s κ); Figure 2B (directional discordance meta-analysis); Figure 3 (PFS meta-analysis); and supplementary analyses covering strict pairing, Fréchet–Hoeffding bounds, small-study effects, and risk of bias.
...
# Table wrapper (accepts 'cap' and 'caption')
...
show_png <- function(dir, file, caption = NULL, width = "90%") {
  ...
  # If PNG path exists → embed normally
...
.export_grviz <- function(g, out_png, width_px = 1800, height_px = 1100) {
  ...
  # Optional Fallback 2 (if forced PNG without rsvg): webshot2
...
mrd_path_candidates <- c(
  file.path("data", "mrd_petct_data.csv"),   
  "mrd_petct_data.csv",
...
# drop NA and empty strings safely
mrd_path_candidates <- mrd_path_candidates[!is.na(mrd_path_candidates) & nzchar(mrd_path_candidates)]

exists_vec <- vapply(mrd_path_candidates, file.exists, logical(1))

if (!any(exists_vec)) {
  knitr::asis_output(
    "<em>mrd_petct_data.csv not found. Put it in <code>data/</code> or set env <code>MRDPET_CSV</code>.</em>"
  )
  knitr::knit_exit()
}
...
mrd_path <- file.path("data", "mrd_petct_data.csv")
stopifnot(file.exists(mrd_path))

# 3) Load and harmonize
mrd <- NULL

if (!is.na(mrd_path) && nzchar(mrd_path) && file.exists(mrd_path) && exists("safe_read_csv")) {
  mrd <- try(safe_read_csv(mrd_path), silent = TRUE)
  if (inherits(mrd, "try-error")) mrd <- NULL
}

if (is.null(mrd)) {
  mrd <- read_csv_flex(mrd_path)  # already clean_names()
}

if (is.null(mrd) || !is.data.frame(mrd)) stop("MRD table not loaded (mrd is NULL).", call. = FALSE)
...
.parse_ci <- function(s) {
  ...
  s <- gsub("\u00b7", ".", s, fixed = TRUE)   # 0·23–0·88 -> 0.23–0.88
...
surv_df <- tibble::tibble(
  Study = tmp$Study,
  tmp$Study[is.na(tmp$Study) | !nzchar(tmp$Study)] <- sprintf("Study_%02d", seq_len(nrow(tmp))),
...
# Coerce list → tibble row if needed
...
# Display priority: HTML -> PNG (best), PDF only for LaTeX/PDF output
if (knitr::is_html_output()) {
  if (file.exists(fig_png)) {
    knitr::include_graphics(fig_png)
  } else if (file.exists(fig_html)) {
...
if (!is.data.frame(src_raw) || !nrow(src_raw)) {
  ...
  if (!all(c("study_id","a","b","c","d","n") %in% names(src))) {
    ...
    # 3) Select and order exactly the columns 
    want <- c(
      "Study","n",
...
```{r frechet-bounds, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
# 1) Pick a concordance-ready source (prefer mrd_conc; return to mrd)
...
if (!nrow(df0)) {
  ...
  # 4)  Point κ and FH bounds per study ----------------------------------
  ...
  fh_tbl <- tibble::tibble(
    ...
    Width  = round(Width, 3),
    po_min  = round(po_min, 4),
    po_max  = round(po_max, 4)
  )
...
if (is.null(df2) || !nrow(df2)) {
  ...
  #  Point kappa
  K_point <- mapply(kappa_from_counts, df2$a, df2$b, df2$c, df2$d)
  #  Bounds; use SIMPLIFY=TRUE then split explicitly
...
# 5) Create FH-bounds figure file if missing (from mrd_primary)
make_fh_fig <- function(df, out = here::here("figs","Supplementary Methods S4-Kappa FH bounds.png")){
  if (!has_cols(df, c("a","b","c","d","n"))) return(FALSE)
  a<-num(df$a); b<-num(df$b); c<-num(df$c); d<-num(df$d); n<-num(df$n)
  ok <- is.finite(a)&is.finite(b)&is.finite(c)&is.finite(d)&is.finite(n)&n>0
  if (!any(ok)) return(FALSE)
  df <- df[ok, , drop=FALSE]
  # labels
  labcol <- intersect(c("Study","StudyShort","study","study_id","id","authors","title"), names(df))
  lab <- if (length(labcol)) as.character(df[[labcol[1]]]) else paste0("Study_", seq_len(nrow(df)))
  # point kappa
  po <- (df$b + df$d)/df$n
  pe <- ((df$a+df$b)/df$n)*((df$a+df$c)/df$n) + ((df$c+df$d)/df$n)*((df$b+df$d)/df$n)
  k_point <- ifelse(is.finite(pe) & (1-pe)>0, (po - pe)/(1 - pe), NA_real_)
  # FH bounds from marginals
  r1 <- df$a + df$b; c1 <- df$b + df$c; r2 <- df$n - r1; c2 <- df$n - c1
  bmin <- pmax(0, r1 - c2, c1 - (df$n - r1))
  bmax <- pmin(r1, c1)
  build_k <- function(bst, i){
    d_star <- r2[i] - (c2[i] - bst)
    a_star <- r1[i] - bst
    c_star <- c1[i] - bst
    po <- (bst + d_star)/df$n[i]
    pe <- (r1[i]/df$n[i])*(c1[i]/df$n[i]) + (r2[i]/df$n[i])*(c2[i]/df$n[i])
    if (!is.finite(pe) || (1-pe)<=0) return(NA_real_)
    (po - pe)/(1 - pe)
  }
  k_low  <- mapply(build_k, bmin, seq_along(bmin))
  k_high <- mapply(build_k, bmax, seq_along(bmax))
  dplot <- tibble::tibble(Study = lab, K_low = k_low, K_high = k_high, K_point = k_point)
  # plot
  dir.create(dirname(out), showWarnings = FALSE, recursive = TRUE)
  suppressPackageStartupMessages(library(ggplot2))
  p <- ggplot2::ggplot(dplot, ggplot2::aes(y = reorder(Study, K_point), x = K_point)) +
    ggplot2::geom_errorbarh(ggplot2::aes(xmin = K_low, xmax = K_high), height = 0.22, alpha = 0.9) +
    ggplot2::geom_point() +
    ggplot2::geom_vline(xintercept = 0, linetype = "dotted") +
    ggplot2::labs(title = "κ identification intervals (FH bounds) with point estimate",
         x = "Cohen’s κ", y = NULL) +
    ggplot2::theme_minimal(base_size = 11)
  ggplot2::ggsave(out, p, width = 7.8, height = 0.5 * nrow(dplot) + 2.0, dpi = 300, bg = "white")
  file.exists(out)
}
fh_png <- here::here("figs","Supplementary Methods S4-Kappa FH bounds.png")
if (!file.exists(fh_png) && is.data.frame(mrd_primary) && nrow(mrd_primary) > 0) {
  invisible(try(make_fh_fig(mrd_primary, fh_png), silent = TRUE))
}
...
data_path <- normalizePath(file.path("tables", "Table_2_Concordance_Augmented.xlsx"))
...
kappa_ci_from_counts <- function(a,b,c,d, B = 1000L){
  ...
  if (!is.finite(se) || !is.finite(lo) || !is.finite(hi)) {
    ...
    if (all(is.finite(p)) && abs(sum(p) - 1) < 1e-8) {
      # small B knit fast; bump if tighter CIs
...
pi_ok <- !inherits(pr,"try-error") && all(is.finite(c(pr$pi.lb, pr$pi.ub)))

p <- ggplot(df_k, aes(y = row_id)) + 
  { if (pi_ok)
      geom_rect(aes(xmin = pr$pi.lb, xmax = pr$pi.ub, ymin = -Inf, ymax = Inf),
                inherit.aes = FALSE, fill = "grey90", alpha = 0.25)
    } +
geom_errorbarh(aes(xmin = k_lo, xmax = k_hi), height = 0.18, linewidth = 0.5) +
...
make_strict <- function(df, max_abs_days = 30, tol_n = 2){
  ...
  if ("simultaneous_mrd_pet_measurement" %in% names(x)) {
    ...
    # B) Fallback Option: Δ-days (pair_days or calculate from dates)
...
pool_row <- function(df){
  ...
  n <- a+b+c+d
  tibble(a=a, b=b, d=d, c=c, N=n, agree = if (n>0) (b+d)/n else NA_real_)
}

r_primary <- pool_row(mrd_primary) %>% mutate(Dataset="PRIMARY")
r_strict  <- if (!inherits(mrd_primary_strict,"try-error") && is.data.frame(mrd_primary_strict) && nrow(mrd_primary_strict)) pool_row(mrd_primary_strict) %>% mutate(Dataset="STRICT TIMEPOINT") else NULL
raw_out   <- bind_rows(r_primary, r_strict) %>% select(Dataset, a, b, d, c, N, agree)
...
# 3. Calculate log-odds etc. using the same helper as PRIMARY
...
if (exists("res_hr") && inherits(res_hr, "rma.uni")) {
  ...
  # if the meta object isn't built for any reason
  note_txt <- paste0(
    "Hazard ratios compare <strong>dual‑negative (MRD−/PET−)</strong> versus <strong>all other states</strong>. ",
...
## PFS pooled summary
```{r pfs-pooled-summary, echo=FALSE, cache=FALSE, results='asis'}
if (exists("surv_df") && is.data.frame(surv_df) && nrow(surv_df) >= 2) {

  fit <- metafor::rma(yi = surv_df$logHR, sei = surv_df$SE, method = "REML", test = "knha")

  # manual 95% prediction interval on HR scale
  mu_hat <- as.numeric(fit$b)
  se_mu  <- as.numeric(fit$se)
  tau2   <- as.numeric(fit$tau2)

  pred_se <- sqrt(se_mu^2 + tau2)
  crit    <- qt(0.975, df = fit$k - 1)  # conservative small-k
  pi_lb   <- exp(mu_hat - crit * pred_se)
  pi_ub   <- exp(mu_hat + crit * pred_se)
  pi_str  <- sprintf("%.2f–%.2f", pi_lb, pi_ub)
  ...
  knitr::asis_output("<em>PFS pooled summary not available: surv_df missing or k&lt;2.</em>")
}
...
txt <- paste0(
  ...
  "Using pooled MRD and PET marginals, we enumerate all feasible 2×2 tables and calculate the true κ. ",
  ...
  "Calculate κ from the full joint 2×2 (or report identified-set bounds)."
)
if (knitr::is_html_output()) {
  ...
  cat(
    ...
    "Using pooled MRD and PET marginals, we enumerate all feasible 2x2 tables and calculate the true kappa. ",
    ...
    "Calculate kappa from the full joint 2x2 (or report identified-set bounds).\n"
  )
}
```
...
if (!build_from_bias) {
  ...
  if (!overwrite && file.exists(quips_path) && file.exists(quadas_path)) {
    message("RoB CSVs already exist; not overwriting. Set options(ROB_OVERWRITE=TRUE) or env ROB_OVERWRITE=1 to recreate")
  } else {
...
if (exists("surv_df") && is.data.frame(rob_quips_overall) && nrow(rob_quips_overall)) {
  # NOTE: surv_df$Study may not  match study_id; if many NAs appear, map IDs upstream.
...
if (exists("conc") && is.data.frame(conc) && nrow(conc)) {
  ...
  if (!is.na(robvar)) {
    ...
    if (inherits(p_base, "ggplot")) {
      # Overlay shape layer mapped to RoB
      p2a_rob <- p_base +
  ggplot2::geom_point(
...
if (!exists("search_sheet")) search_sheet <- tibble::tibble(database=c("MEDLINE","Cochrane","Others"),
                                                            strategy=c("[Add final MEDLINE query]","[Add Cochrane query]","[Add other sources]"))
...
# Legacy splits (only if writexl present)
if (requireNamespace("writexl", quietly = TRUE)) {
  ...
  if (knitr::is_html_output()) {
    knitr::asis_output('
    <style>.dl-btn{display:inline-block;padding:.45em .8em;border:1px solid #ddd;border-radius:8px;text-decoration:none;font-weight:600;font-size:.95em;background:#f7f7f7;color:#111}.dl-btn:hover{background:#eee}</style>')
    knitr::asis_output(sprintf(
      '<p><a class="dl-btn" href="%s" download>Download MASTER (xlsx)</a>
          <a class="dl-btn" href="%s" download>ST3 (xlsx)</a>
          <a class="dl-btn" href="%s" download>ST4 (xlsx)</a></p>',
      file.path("tables", "Supplementary_Tables_MASTER.xlsx"),
      file.path("tables", "Supplementary_Table_ST3_Survival_and_Meta.xlsx"),
      file.path("tables", "Supplementary_Table_ST4_Methods_Checklists.xlsx")
    ))
  }
}
